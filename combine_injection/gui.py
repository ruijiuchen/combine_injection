...
<<<<<<< HEAD
    ...
    create_and_fill_TH2F(injection_list_path, injection_name_new, file_path, frameID, frameID - frameID_offset, frameID - frameID_offset+ frameID_range)
...
<<<<<<< HEAD
    ...
    print(" plot_option = ",plot_option, "type =", type(plot_option))
    if plot_option:
...
<<<<<<< HEAD
            ...
            #amp = np.fromfile(bin_amp_path, dtype=np.float32)
            amp = np.fromfile(bin_amp_path, dtype=np.float32).reshape(-1, fre.size)  # Reshape amp to 2D array if needed
            print("Calculating average amplitude.")
...
<<<<<<< HEAD
            ...
            combine_tdms(file_dir, file_start, file_stop, average_number, fre_min, fre_range, date_start, plot_option)
...
def main():
    ...
    if len(args) == 1 and args[0].endswith(".toml"):
        # Read parameters from TOML
        gui.config_file = args[0]
        gui.load_parameters()
        gui.start_program()
        sys.exit(0)

    elif len(args) >= 1:
        # Use command-line arguments to populate GUI fields
        mode = args[0]
        gui.mode_combo.setCurrentText(mode)
        gui.update_ui_for_mode()
        param_names = [param for param, _ in gui.param_entries]
        for i, (_, entry) in enumerate(gui.param_entries):
            if i + 1 < len(args):
                entry.setText(args[i + 1])
        gui.start_program()
        sys.exit(0)

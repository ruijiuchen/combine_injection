...
def main():
    ...
    if mode == "accumulate":
        ...
        if len(sys.argv) != 9:
            ...
            print("Usage: combine_injection  <parameters.toml>")
            sys.exit(1)
        fir_dir       =       sys.argv[2]
    ...
    elif mode.endswith(".toml"):
        config = toml.load(mode)
        if "mode" not in config:
            print("Invalid TOML file: 'mode' field is missing")
            sys.exit(1)
        ...
        if mode == "accumulate":
            ...
            dtime = float(config["dtime"])
            filtered_files = filter_files_by_time(path_to_files, prefix, extension, start_time, end_time, "%Y.%m.%d.%H.%M.%S.%f")
            read_fft_injection_from_files(filtered_files, extension, dtime)
        elif mode == "get_full_spec":
            injection_list_path = config["injection_list_path"]
            base_path = config["base_path"]
            max_workers = int(config["max_workers"])
            frameID_offset = int(config["frameID_offset"])
            frameID_range = int(config["frameID_range"])
            injections = read_injection_list(injection_list_path)
            process_injections_concurrently(injection_list_path, injections, base_path, process_injection, max_workers, frameID_offset, frameID_range)
        elif mode == "convert_npz":
            bin_fre_path = config["bin_fre_path"]
            bin_time_path = config["bin_time_path"]
            bin_amp_path = config["bin_amp_path"]
            start_time = time.time()
            bin_npz_path = bin_fre_path.replace(".bin_fre", ".npz")
            fre = np.fromfile(bin_fre_path, dtype=np.float64)
            time_data = np.fromfile(bin_time_path, dtype=np.float32)
            ...
            file_start = int(config["file_start"])
            file_stop = int(config["file_stop"])
            average_number = int(config["average_number"])
            fre_min = float(config["fre_min"])
            fre_range = float(config["fre_range"])
            date_start = config["date_start"]
            combine_tdms(file_dir, file_start, file_stop, average_number, fre_min, fre_range, date_start)
        else:
            print(f"Invalid mode in TOML file: {mode}")
            sys.exit(1)
